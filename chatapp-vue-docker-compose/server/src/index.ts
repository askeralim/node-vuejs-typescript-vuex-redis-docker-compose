...
subscriber.on("message", function (channel: Channel, message: any) {
    ...
    if (channel == Channel.USER) {
        ...
        redisClient.lrange("chatRoomList", 0, -1, function (err: any, targetList: any) {
            const toList = targetList.map((usr:any) => JSON.parse(usr)).filter((user: any) => { console.log("User Id:" + user.id + " Message To ID:" + msg.toId); return user.id == msg.toId });
            toList.forEach((user:any) => {
                if(user.type == RoomType.USER){
                    console.log("Sending Message to User  :"+JSON.stringify(user));
                    io.to(user.socketId).emit('new_message', message);
                    const fromList = targetList.map((usr:any) => JSON.parse(usr)).filter((user: any) => { console.log("User Id:" + user.id + " Message To ID:" + msg.toId); return  user.id == msg.fromId});
                    fromList.forEach((user:any) =>{
                        io.to(user.socketId).emit('new_message', message);
                    });
                }else{
                    ...
                    redisClient.lrange(user.name, 0, -1, function (err: any, userList: any) {
                        userList.forEach((user:any)=>{
                            const targetUser:any = JSON.parse(user);
...
io.on('connection', function (socket: any) {
    ...
    socket.on('newUser', (userData: any) => {
        console.log("NEW USER :" + userData.name + " User Type:" + userData.type);
        const userId = userData.name;//uuidV4();
        const user = new User(socket.id, userId, userData.name, new Date(), userData.type);//userObj.Name;//uuidV4();
        ...
        redisClient.lrange("chatRoomList", 0, -1, function (err: any, list: any) {
            list = list.map((user: any) => JSON.parse(user));
            socket.user = user;
    ...
    socket.on('newChatRoom', (room: any) => {
        const chatRoomId = room.name;//uuidV4();
        const chatRoom = new User('', chatRoomId, room.name, new Date(), RoomType.ROOM);//userObj.Name;//uuidV4();
    ...
    socket.on('joinRoom', (joining: any) => {
        //console.log("Message : "+JSON.stringify(message))
    ...
    socket.on('leaveRoom', (leave: any) => {
        //console.log("Message : "+JSON.stringify(message))
        //room.time = dateFormat(new Date(), "ddd h:MM:ss");
        // publisher.publish(Channel.MESSAGE, JSON.stringify(room));
        console.log('Leave : Socket :'+ socket.id+' room Name :'+leave.id);
        redisClient.del(JSON.stringify({ServerId:SERVER_ID,SocketId:socket.id}),function(err:any, reply:any){
